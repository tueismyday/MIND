...
from config.settings import EMBEDDING_MODEL_NAME, EMBEDDING_DEVICE, GENERATION_TEMPERATURE

LLM = OllamaLLM(
    model="gemma3:4b-16k-it",
...
def generate_summary(text: str, llm=LLM) -> str:
    ...
    """
    prompt = f"Sammenfat fÃ¸lgende dokument i en kort, klinisk relevant opsummering pÃ¥ dansk:\n\n{text}"
    response = LLM.invoke(
        prompt,
        )
...
def main() -> None:
    ...
    Returns:
        None
    """
    data_dir = os.path.join(os.path.dirname(__file__), "data/generated_documents")
    db_dir = os.path.join(os.path.dirname(__file__), "data/generated_documents_db")
